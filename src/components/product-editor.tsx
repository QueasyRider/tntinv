...
export function ProductEditor({ product, activities, onBack, onSave, onExport, busy }: {
  ...
  const issues = [
    ...
    ...(!draft.squareCategoryId ? ["Missing Square category mapping"] : []),
  ];
  ...
  return <main className="editor-page">
    ...
    {tab === "edit" && <>
      ...
      <div className="editor-grid">
        ...
        <aside className="preview-rail"><header><Square size={20} fill="currentColor" /><h2>SQUARE PREVIEW</h2></header><div className="preview-product"><div className="preview-image" style={{ backgroundImage: `url(${draft.images[0]})` }}></div><div><h3>{draft.title}</h3><strong>${(draft.priceCents / 100).toFixed(2)}</strong><span>{draft.category}</span></div></div><p>{draft.description}</p><h3>Variations ({draft.variants.length || 1})</h3><div className="preview-variants">{(draft.variants.length ? draft.variants : [{ id: "regular", name: "Regular", quantity: draft.quantity, priceCents: draft.priceCents }]).map((variant) => <div key={variant.id}><span>{variant.name}</span><span>${(variant.priceCents / 100).toFixed(2)}</span><span>In stock: {variant.quantity}</span></div>)}</div><div className="total"><strong>Total inventory</strong><strong>{draft.variants.length ? draft.variants.reduce((sum, item) => sum + item.quantity, 0) : draft.quantity}</strong></div>{issues.length > 0 && <div className="validation"><AlertTriangle size={20} /><div><strong>{issues.length} validation {issues.length === 1 ? "issue" : "issues"}</strong><span>{issues[0]}</span></div></div>}<div className="mapping"><h3>Listing mapping</h3><div><span>Etsy listing ID</span><a href={`https://www.etsy.com/listing/${product.etsyListingId}`} target="_blank" rel="noreferrer">#{product.etsyListingId}</a></div><div><span>Square item ID</span><strong>{product.squareItemId || "Not exported yet"}</strong></div></div></aside>
      </div>
      <section className="variants-panel"><header><h2>Variants &amp; options</h2><button className="button outline small" onClick={() => set("variants", [...draft.variants, { id: crypto.randomUUID(), name: "New option", optionName: "Option", optionValue: "Value", sku: `${draft.sku}-${draft.variants.length + 1}`, priceCents: draft.priceCents, quantity: 0 }])}>+ Add variant</button></header><div className="variant-table"><div className="variant-row head"><span className="variant-thumb-cell">Image</span><span>Option name</span><span>Value</span><span>SKU</span><span>Price</span><span>Quantity</span><span></span></div>{draft.variants.map((variant, index) => <div className="variant-row" key={variant.id}><span className="variant-thumb-cell"><span className="variant-thumb" role="img" aria-label={`${variant.name} thumbnail`} style={imageStyle(draft.images[index] || draft.images[0] || product.original.images[index] || product.original.images[0])}></span></span><input value={variant.optionName} onChange={(event) => updateVariant(variant.id, "optionName", event.target.value)} /><input value={variant.optionValue} onChange={(event) => { updateVariant(variant.id, "optionValue", event.target.value); updateVariant(variant.id, "name", event.target.value); }} /><input value={variant.sku} onChange={(event) => updateVariant(variant.id, "sku", event.target.value)} /><input type="number" value={(variant.priceCents / 100).toFixed(2)} onChange={(event) => updateVariant(variant.id, "priceCents", Math.round(Number(event.target.value) * 100))} /><input type="number" value={variant.quantity} onChange={(event) => updateVariant(variant.id, "quantity", Number(event.target.value))} /><button onClick={() => set("variants", draft.variants.filter((item) => item.id !== variant.id))} aria-label="Remove variant"><X size={16} /></button></div>)}</div></section>
    </>}

    {tab === "preview" && <section className="diff-view"><div className="diff-heading"><div><h2>ETSY ORIGINAL</h2><span>Read only</span></div><ArrowLeft size={24} className="diff-arrow" /><div><h2>SQUARE-READY VERSION</h2><span>{differences.length} fields changed</span></div></div>{(["title", "description", "priceCents", "sku", "category", "tags", "quantity"] as const).map((key) => <div className={`diff-row ${differences.includes(key) ? "changed" : ""}`} key={key}><strong>{key === "priceCents" ? "Price" : key[0].toUpperCase() + key.slice(1)}</strong><div>{Array.isArray(product.original[key]) ? product.original[key].join(", ") : key === "priceCents" ? `$${(Number(product.original[key]) / 100).toFixed(2)}` : String(product.original[key])}</div><div>{Array.isArray(draft[key]) ? (draft[key] as string[]).join(", ") : key === "priceCents" ? `$${(Number(draft[key]) / 100).toFixed(2)}` : String(draft[key])}</div></div>)}</section>}
