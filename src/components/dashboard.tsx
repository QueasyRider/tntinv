...
import { AlertTriangle, ArrowUpRight, Box, Check, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, Download, Edit3, ExternalLink, Filter, PackageCheck, Search, SquareArrowOutUpRight, Upload, X } from "lucide-react";
...
export function Dashboard({ state, selected, setSelected, onOpen, onImport, onExport, onBulk, busy, inventoryOnly = false }: {
  ...
  const products = useMemo(() => state.products.filter((product) => {
    ...
    const matches = !query || [product.working.title, product.working.sku, product.etsyListingId].some((value) => value.toLowerCase().includes(query));
  ...
  return <div className="dashboard-grid"><main className="dashboard-main">
    ...
    <section className="table-workspace">
      <div className="table-tools"><label className="search"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search title, SKU, listing ID" /></label><div className="filters" aria-label="Product status filter">{(["all", "ready", "needs_review", "exported", "error"] as const).map((id) => <button key={id} className={filter === id ? "active" : ""} onClick={() => setFilter(id)}>{id === "all" ? `All products (${state.metrics.imported})` : `${statusLabel[id]} (${state.products.filter((product) => product.status === id).length})`}</button>)}</div><button className="filter-icon" aria-label="More filters"><Filter size={17} /></button></div>
      ...
      <div className="table-scroll"><table><thead><tr><th className="check-cell"><input aria-label="Select all visible products" type="checkbox" checked={allSelected} onChange={() => setSelected(allSelected ? new Set() : new Set(products.map((product) => product.id)))} /></th><th>Product</th><th>Etsy listing</th><th>SKU</th><th>Price</th><th>Qty</th><th>Variants</th><th>Status</th><th>Updated</th><th></th></tr></thead><tbody>{products.map((product) => <tr key={product.id} className={selected.has(product.id) ? "selected" : ""}>
        ...
        <td><button className="product-cell" onClick={() => onOpen(product)}><span className="thumb" style={{ backgroundImage: `url(${product.working.images[0]})` }}>{product.working.title.slice(0, 1)}</span><span><strong>{product.working.title}</strong><small>{product.working.category} · {product.working.state}</small></span></button></td>
        <td><a href={`https://www.etsy.com/listing/${product.etsyListingId}`} target="_blank" rel="noreferrer">#{product.etsyListingId}<ExternalLink size={12} /></a></td><td className="mono">{product.working.sku}</td><td>${(product.working.priceCents / 100).toFixed(2)}</td><td>{product.working.quantity}</td><td>{product.working.variants.length ? product.working.variants.map((variant) => variant.optionValue).join(", ") : "—"}</td><td><Status status={product.status} /></td><td><span className="date">{new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(product.updatedAt))}<small>{new Intl.DateTimeFormat("en", { hour: "numeric", minute: "2-digit" }).format(new Date(product.updatedAt))}</small></span></td><td><button className="row-go" onClick={() => onOpen(product)} aria-label={`Edit ${product.working.title}`}><ChevronRight size={18} /></button></td>
      ...
      <footer className="table-footer"><span>Showing {products.length} of {state.products.length} products</span><div><button disabled><ChevronLeft size={15} /></button><button className="active">1</button><button disabled><ChevronRight size={15} /></button></div></footer>
    </section>
