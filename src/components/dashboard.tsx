...
export function Dashboard({ state, selected, setSelected, onOpen, onImport, onExport, onBulk, onHistory, onPreflight, busy, inventoryOnly = false }: {
  ...
  </main>{!inventoryOnly && <aside className="activity-rail"><h2>RECENT ACTIVITY</h2>{state.activities.slice(0, 8).map((activity) => <ActivityRow key={activity.id} activity={activity} />)}<button className="history-link" onClick={onHistory}>View all history <ArrowUpRight size={15} /></button><div className="rail-stamp" aria-hidden="true"></div></aside>}</div>;
}
